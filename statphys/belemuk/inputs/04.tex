...
Теперь будем рассматривать квантовый газ атомов в магнитном поле. Каждый атом имя собственный момент $J$, проекция которого на ось магнитного поля $z$ будет квантоваться по $\nu = - J, \ldots, + J$.
В магнитном поле они приобретают добавку к энергии $\Delta E_\nu = g_J \mu_B H \nu$.
\begin{equation*}
	Z_1 = \sum_{\text{кв. сост.}} e^{\Delta E_\nu /T} = \sum_{\nu = J}^{+J} e^{- \frac{g_J \mu_B H \nu}{T}} = \sum_\nu e^{- \alpha \nu} = e^{\alpha J} [1 + e^{- \alpha} + e^{- 2\alpha} + \ldots + e ^{-2 \alpha J}].
\end{equation*}
...
\begin{equation*}
	...
	= T N \frac{1}{Z_1} \frac{\partial Z_1}{\partial \alpha} \frac{\partial \alpha}{\partial H} = T N \frac{g_J \mu_B}{T} \frac{1}{Z_1} \frac{\partial Z_1}{\partial \alpha}.
\end{equation*}
...
\begin{equation*}
	...
	\equiv B_J(\alpha),
\end{equation*}
где $B_J(\alpha)$ -- функция Бриуллена\footnote{за график спасибо Саше Яворскому.}.
\begin{figure}[h]
    \centering
...
В слабых полях 
\begin{equation*}
	M^z = N g_J \mu_B \frac{J(J+1)}{3} \frac{g_J \mu_B H}{T} = N \frac{(g_J \mu_B)^2}{3 T}J (J+1) H.
\end{equation*}
\begin{itemize}
	\item  $g_J m_B H \ll T$ -- слабое поля ($\alpha \ll 0$): $\chi = N \frac{(g_J\mu_B)^2}{3 T}J (J+1) \sim \frac{1}{T}$ -- закон Кюри;
	\item $g_J m_B H \gg T$ -- сильное поле ($\alpha \gg 0$): $M^{z} = g_J \mu_B N J$.
\end{itemize}
