\textbf{Т24}. Составим табличку, из неё найдём $P(\xi \eta = 0) = 1$ и $P(\xi = \eta = 0) = 0$. 


\textbf{Т25}. Для $\eta$ можем написать, что $p_\eta (x) = \frac{1}{3}\left(\delta(x-1) + \delta(x) + \delta(x+1)\right)$, тогда по формуле свёртки
\begin{equation*}
	F_{\xi + \eta} (x) = \int_{-\infty}^{x} \frac{1}{3}\left(
		f_\xi(t-1) + f_\xi(t) + f_\xi (t+1)
	\right) \d t.
\end{equation*}
